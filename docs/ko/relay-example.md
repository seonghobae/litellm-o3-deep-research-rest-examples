...
- `RELAY_TIMEOUT_SECONDS` (선택, 기본 `30`) — Chat Completions 턴 타임아웃
- `RELAY_RESEARCH_TIMEOUT_SECONDS` (선택, 기본 `300`) — deep_research 실행 타임아웃 (`/api/v1/chat` 자동 tool calling 시 deep_research는 이 값을 사용)
- `LITELLM_CHAT_MODEL` (선택, 기본 `gpt-4o`) — `POST /api/v1/chat` orchestration에 사용하는 모델 (function calling 지원 모델이어야 함)
