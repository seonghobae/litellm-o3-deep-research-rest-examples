...
## 5. 현재 예제가 background 설정인가요?

아니요. **현재 예제는 background 서비스가 아니라 foreground 1회성 실행 CLI**입니다.
...
현재 저장소는 background 모드를 **기본 제공하지 않습니다**.
...
- 현재 구현은 background 서비스가 아니라 **요청 1회 실행 후 종료되는 CLI 도구**입니다.
