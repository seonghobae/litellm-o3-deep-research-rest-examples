...
클라이언트가 OpenAI Responses API function-calling 2-step 흐름을 직접 처리합니다. 첫 번째 `POST /v1/responses`에서 `function_call`을 받고, relay `POST /api/v1/tool-invocations`로 실제 research를 실행한 뒤, `previous_response_id`와 `function_call_output`으로 두 번째 `POST /v1/responses`를 보냅니다.
...
단, 이 endpoint는 relay 예제용 helper이며 OpenAI 표준 API surface는 아닙니다.
...
| 환경변수 | 기본값 | 적용 대상 |
|---------|-------|---------|
| `RELAY_TIMEOUT_SECONDS` | `30` | Responses orchestration turns |
