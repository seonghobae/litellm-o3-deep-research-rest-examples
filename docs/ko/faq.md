...
모델(gpt-4o)이 Responses API의 `function_call` 출력으로 "이 질문은 deep_research가 필요하다"고 판단하면, direct client가 relay `POST /api/v1/tool-invocations`로 deep_research를 실행하고 `function_call_output` + `previous_response_id`로 두 번째 `POST /v1/responses`를 보내 최종 답변을 합성합니다. relay 서버가 실행 중이어야 합니다.
...
`/api/v1/chat` 자동 tool calling에서 two-stage 타임아웃을 사용합니다. Responses orchestration turn(30초 기본)과 deep_research 실행(300초 기본)을 분리했습니다. `o3-deep-research`는 최대 10분 이상 걸릴 수 있으므로 필요시 `RELAY_RESEARCH_TIMEOUT_SECONDS=600`으로 늘리세요.
