...
모델(gpt-4o)이 Responses API의 `function_call`로 "이 질문은 deep_research가 필요하다"고 판단하면 relay `POST /api/v1/tool-invocations`를 통해 deep_research를 자동 실행하고, 이어서 `function_call_output`으로 최종 답변을 합성합니다. relay 서버가 실행 중이어야 합니다.
