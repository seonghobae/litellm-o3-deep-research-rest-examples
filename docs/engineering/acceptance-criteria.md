...
3. Each client has automated tests covering configuration loading, URL normalization, successful `chat/completions` and `responses` calls, and non-2xx errors.
...
6. If live credentials are available, at least one successful non-streaming call is observed for each supported API style in each language before the work is declared complete.
