...
6. Korean user-facing manuals are present and GitHub Pages-publishable.
7. The docs site builds successfully with `mkdocs build --strict`.
8. Local verification includes:
    - `cd clients/python && uv run pytest`
    - `cd clients/java && mvn test`
    - `mkdocs build --strict`
9. If live credentials are available, at least one successful non-streaming foreground call is observed for each supported API style in each language, and at least one successful `background: true` response submission is observed in each language before the work is declared complete.
